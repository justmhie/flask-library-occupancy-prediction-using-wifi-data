...
**File**: `train_granular_ablation_study.py`
...
**File**: `student_survey_validation.py`
...
import subprocess
subprocess.run(['python', 'train_granular_ablation_study.py'])
...
subprocess.run(['python', 'train_with_exam_awareness.py'])
...
| File | Purpose | Output |
|------|---------|--------|
| `train_granular_ablation_study.py` | Test individual feature impact | JSON results, visualization |
| `exam_period_tagger.py` | Tag exam periods | Tagged dataframe, JSON config |
| `train_with_exam_awareness.py` | Train exam-aware models | Models, comparison plots |
| `student_survey_validation.py` | Survey analysis | Templates, guides, visualizations |
| `wifi_rfid_correlation.py` | Validate vs RFID | Correlation plots, report |
