...
**Location:** Put it in the project root (same folder as the main README)
...
### Main Scripts
- `scripts/train_multiple_model_types.py` - Train models, save locally
- `scripts/train_multiple_model_types_supabase.py` - Train models, upload to cloud
- `api_backend.py` - Backend API server
- `library-dashboard/` - React frontend dashboard

### Supabase Scripts (Optional)
- `scripts/setup_supabase_storage.py` - Create cloud storage bucket
- `scripts/download_models_from_supabase.py` - Download models from cloud
- `scripts/migrate_to_supabase.py` - Upload existing data to cloud
...
**Solution:** Run training first: `python scripts/train_multiple_model_types.py`
...
2. Run setup: `python scripts/setup_supabase_storage.py`
...
2. **[README.md](../README.md)** - Complete documentation (project root)
...
2. Train models → python scripts/train_multiple_model_types.py
