...
1. Edit `scripts/train_multiple_model_types_supabase.py`:
...
```python
# In scripts/setup_supabase_storage.py, modify:
BUCKETS = [
    {
...
1. **scripts/setup_supabase_storage.py** - Creates storage bucket (one-time)
2. **scripts/train_multiple_model_types_supabase.py** - Trains and uploads to Supabase
3. **scripts/download_models_from_supabase.py** - Downloads models from Supabase
